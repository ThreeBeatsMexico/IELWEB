        ...
        public UsuariosBE addUsuario(ReglasBE Reglas, UsuariosBE Usuario, List<DomicilioBE> Domicilios, List<ContactoBE> Contactos, List<RolesXUsuarioBE> RolesXUsuario, Int64 App)
        {
            ...
            try
            {
                using (scopeGrl)
                {
                    ...
                    var USUARIO = dboSegLatino.sp_insUser(Usuario.IDAPLICACION, (Usuario.IDSEXO > 0 ? (Int32?)Usuario.IDSEXO : null), (Usuario.IDTIPOPERSONA > 0 ? (Int32?)Usuario.IDTIPOPERSONA : null), (Usuario.IDESTADOCIVIL > 0 ? (Int32?)Usuario.IDESTADOCIVIL : null), (Usuario.IDAREA > 0 ? (Int32?)Usuario.IDAREA : null), (Usuario.IDTIPOUSUARIO > 0 ? (Int32?)Usuario.IDTIPOUSUARIO : null), Usuario.IDUSUARIOAPP, Usuario.APATERNO, Usuario.AMATERNO, Usuario.NOMBRE, Usuario.FECHANACCONST, Usuario.USUARIO, Usuario.PASSWORD, Usuario.RUTAFOTOPERFIL, DateTime.Now, Usuario.ACTIVO, (Reglas.IDUSRMODIF > 0 ? (Int64?)Reglas.IDUSRMODIF : null), App);
                    Int64 IdUsuario = 0;
                    foreach (var s in USUARIO)
                    {
                        IdUsuario = Int64.Parse(s.SCOPE_IDENTITY.ToString());
                    ...
                    {
                        dboSegLatino.spAddUsuarioXAplicacion(long.Parse(Rol.IDAPLICACION), IdUsuario, true);
                    }
                    //}
                    ...
                    {
                        dboSegLatino.sp_insRolXUserApp(Rol.IDROL, IdUsuario, (Rol.IDESTACIONXAPP > 0 ? (Int64?)Rol.IDESTACIONXAPP : null), (Rol.IDROLXUSUARIO > 0 ? (Int64?)Rol.IDROLXUSUARIO : null), Rol.ACTIVO);
                    }
